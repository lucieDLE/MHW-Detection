...
class SSTADataset(Dataset):
    ...
    def __init__(
        ...
        land_mask = xr.open_zarr(str(landmask_path))
        land_mask = land_mask.coarsen(lat=coarsen_factor, lon=coarsen_factor, boundary="trim").mean().land_mask.values.astype(bool)
...
def make_loaders(
    ssta_path: str | Path = config.SSTA_DAILY_PATH,
    landmask_path: str | Path = config.LANDMASK_PATH,
    n_in: int = config.DL_N_IN,
    n_out: int = config.DL_N_OUT,
    batch_size: int = config.BATCH_SIZE,
    num_workers: int = config.NUM_WORKERS,
    coarsen_factor: int = config.DL_COARSEN_FACTOR
):
    train_ds = SSTADataset(ssta_path, landmask_path, config.DL_TRAIN_RANGE, n_in, n_out, coarsen_factor)
    val_ds   = SSTADataset(ssta_path, landmask_path, config.DL_VAL_RANGE,   n_in, n_out, coarsen_factor)
    test_ds  = SSTADataset(ssta_path, landmask_path, config.DL_TEST_RANGE,  n_in, n_out, coarsen_factor)
